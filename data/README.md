...
| `career.json` | Match history and legacy ranking events | `app/career/page.tsx` |
| `rankings.json` | Canonical ATP singles/doubles ranking snapshots | `app/page.tsx`, `app/career/page.tsx`, `app/tennis-level-comparison/page.tsx` |
...
- `physiology.json` — updated by `.github/workflows/update_physiology.yml` (manual, WHOOP API)
- `rankings.json` and `../automation/state/ranking_alerts.json` — updated by `.github/workflows/update_rankings.yml` (weekly); the outbox state is committed before and after alert delivery
